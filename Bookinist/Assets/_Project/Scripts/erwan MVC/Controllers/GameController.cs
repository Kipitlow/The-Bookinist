...
public class GameController : MonoBehaviour
{
    ...
    private ItemModel _selectedCard;
    ...
    public void OnCardSelected(ItemModel item)
    {
        bool isValidSelection = (_gameModel.IsPlayerOneTurn && item.IsPlayer1Card) 
            || (!_gameModel.IsPlayerOneTurn && !item.IsPlayer1Card);
        ...
        _gameModel.SelectedCard = item;

        if (item.IsPlayer1Card)
            _inventoryControllerPlayer1.SelectCard(item);
        else
            _inventoryControllerPlayer2.SelectCard(item);
