...
/// <summary>
/// Outils d'édition en runtime : palette, sélection, preview (ghost), placement et effacement.
/// </summary>
public class LevelEditor : MonoBehaviour
{
    ...
    public LayerGrid ActiveLayer => (_layers != null && _activeLayerIndex >= 0 && _activeLayerIndex < _layers.Count) ? _layers[_activeLayerIndex] : null;
    public GameObject SelectedPrefab => (_palette != null && _selectedPaletteIndex >= 0 && _selectedPaletteIndex < _palette.Count) ? _palette[_selectedPaletteIndex] : null;
    ...
    private void CreateInputActions()
    {
        ...
        _scrollAction = new InputAction("Scroll", InputActionType.Value, expectedControlType: "Vector2");
        ...
        _mousePositionAction = new InputAction("MousePosition", InputActionType.Value, expectedControlType: "Vector2");
    ...
    private void OnNextPrefab(InputAction.CallbackContext ctx) => SetSelectedPrefab((_selectedPaletteIndex + 1) % Mathf.Max(1, _palette.Count));
    private void OnPrevPrefab(InputAction.CallbackContext ctx) => SetSelectedPrefab((_selectedPaletteIndex - 1 + _palette.Count) % Mathf.Max(0, _palette.Count));
    ...
    private void SpawnGhost()
    {
        ...
        if (!layerExists)
            Debug.LogError($"[LevelEditor] Le Sorting Layer {GhostSortingLayer} n'existe pas.", this);
    ...
    {
        if (_ghost == null || ActiveLayer == null) { SetGhostVisible(false); return; }

        Vector3? worldPos = RaycastOntoLayerPlane(ActiveLayer);
        if (worldPos == null) { SetGhostVisible(false); _ghostCell = null; return; }

        Vector3? snapped = ActiveLayer.SnapToGrid(worldPos.Value);
        _ghostCell = ActiveLayer.WorldToCell(worldPos.Value);
        if (snapped == null) { SetGhostVisible(false); _ghostCell = null; return; }

        _ghost.transform.position = snapped.Value;

        bool cellFree = _ghostCell.HasValue && ActiveLayer.IsCellFree(_ghostCell.Value);
        SetGhostColor(cellFree ? new Color(1f, 1f, 1f, _ghostAlpha) : new Color(1f, 0.2f, 0.2f, _ghostAlpha));
        SetGhostVisible(true);
    }

    private void SetGhostVisible(bool visible) { if (_ghost != null) _ghost.SetActive(visible); }

    private void SetGhostColor(Color color)
    {
        foreach (var sr in _ghost.GetComponentsInChildren<SpriteRenderer>()) sr.color = color;
    ...
    public void RegisterLayer(LayerGrid layer) { if (!_layers.Contains(layer)) _layers.Add(layer); }
