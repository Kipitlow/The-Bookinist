    ...
    */
        ...
        if (_poidAutoriser < 3)
        {
            ...
            switch (Context.itemName)
            {
                ...
                case "Citron" or "Figue" or "Orange" or "Pomme":
                    ...
                    break;
                case "GrosPot" or "PetitPot" or "vase" or "Vase": //Pug SpriteRenderer Vase: n'apparait pas
    ...
    IEnumerator _setItem()
    {
        ...
        if (_inventoryController != null)
        {
            ...
            {
                Debug.Log($"Place: {_inventoryController.IsInventoryHasPlace()}");
                if (_listPoids[i] != null && _stockIteam[i] != null && _inventoryController.IsInventoryHasPlace())//&& inventoryController._inventoryContents
                {
                    _inventoryController.AddInventoryItem(_stockIteam[i]);
                    Destroy(_listPoids[i]);
                    _poidAutoriser -= 1;
                }
                Destroy(_2emePoids);
